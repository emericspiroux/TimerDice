...
function getNextVersion(currentVersion, type) {
  ...
  if (versionSplit.length !== 3)
    return console.log("Package version not npm valid");
  switch (type) {
    case "minor":
...
function updatePackageVersion(filepath, newVersion) {
  ...
  if (!fs.existsSync(relativeFilePath))
    return console.log("Unable to find :", filepath);
  let file = require(relativeFilePath);
  if (!(file && file.version))
    return console.log(`${filepath} not a package json`);
...
if (!options.t && !options._.length) {
  return console.log(
    "Usage : publish [-t|--type=<minor|major>] source_and_target_package.json ... other_target_package.json... \n\nUpdate number version."
  );
}
...
for (const packageFilePath of options._) {
  ...
  if (!updatePackageVersion(packageFilePath, nextVersion)) continue;
  else updated++;
}
